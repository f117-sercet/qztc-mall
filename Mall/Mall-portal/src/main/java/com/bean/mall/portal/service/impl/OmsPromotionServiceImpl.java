...
import com.bean.model.PmsProductFullReduction;
import com.bean.model.PmsProductLadder;
...
public class OmsPromotionServiceImpl implements OmsPromotionService {
    ...
    @Override
    public List<CartPromotiomItem> calcCartPromotion(List<OmsCartItem> cartItemList) {
      //先根据怕productId对CartItem进行分组，以spu为单位进行计算优惠。
        //1.先根据productId对CartItem进行分组，以spu为单位进行计算优惠
        Map<Long, List<OmsCartItem>> productCartMap = groupCartItemBySpu(cartItemList);
        //2.查询所有商品的优惠相关信息
        List<PromotionProduct> promotionProductList = getPromotionProductList(cartItemList);
        //3.根据商品促销类型计算商品促销优惠价格
        List<CartPromotiomItem> cartPromotionItemList = new ArrayList<>();
        for (Map.Entry<Long, List<OmsCartItem>> entry : productCartMap.entrySet()) {
            ...
            if (promotionType == 1) {
                ...
                for (OmsCartItem item : itemList) {
                    CartPromotiomItem cartPromotionItem = new CartPromotiomItem();
     ...
     * 对没满足优惠条件的商品进行处理
     */
    private void handleNoReduce(List<CartPromotiomItem> cartPromotionItemList, List<OmsCartItem> itemList,PromotionProduct promotionProduct) {
        for (OmsCartItem item : itemList) {
            CartPromotiomItem cartPromotionItem = new CartPromotiomItem();
            BeanUtils.copyProperties(item,cartPromotionItem);
